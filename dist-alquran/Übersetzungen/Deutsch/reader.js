(function () {
  ...
  if (isChapter) {
    ...
    function findTopVerse() {
      var els = document.querySelectorAll('.vb[id], .verse[id]');
    ...
    s.textContent =
      '.vb,.verse{cursor:pointer;transition:background .18s;position:relative}' +
      '.vb:hover,.verse:hover{background:rgba(184,150,46,.06)}' +
    ...
    document.addEventListener('click', function(e) {
      var el = e.target.closest('.vb, .verse');
      if (!el || !el.id) return;
