...
// ENGLISCHE Surah-Namen (für .rs - oben)
const ENGLISH_NAMES = ["Al-Fatihah","Al-Baqarah","Ali 'Imran","An-Nisa","Al-Ma'idah","Al-An'am","Al-A'raf","Al-Anfal","At-Tawbah","Yunus","Hud","Yusuf","Ar-Ra'd","Ibrahim","Al-Hijr","An-Nahl","Al-Isra","Al-Kahf","Maryam","Ta-Ha","Al-Anbiya","Al-Hajj","Al-Mu'minun","An-Nur","Al-Furqan","Ash-Shu'ara","An-Naml","Al-Qasas","Al-'Ankabut","Ar-Rum","Luqman","As-Sajdah","Al-Ahzab","Saba","Fatir","Ya-Sin","As-Saffat","Sad","Az-Zumar","Ghafir","Fussilat","Ash-Shura","Az-Zukhruf","Ad-Dukhan","Al-Jathiyah","Al-Ahqaf","Muhammad","Al-Fath","Al-Hujurat","Qaf","Ad-Dhariyat","At-Tur","An-Najm","Al-Qamar","Ar-Rahman","Al-Waqi'ah","Al-Hadid","Al-Mujadilah","Al-Hashr","Al-Mumtahanah","As-Saff","Al-Jumu'ah","Al-Munafiqun","At-Taghabun","At-Talaq","At-Tahrim","Al-Mulk","Al-Qalam","Al-Haqqah","Al-Ma'arij","Nuh","Al-Jinn","Al-Muzzammil","Al-Muddaththir","Al-Qiyamah","Al-Insan","Al-Mursalat","An-Naba","An-Nazi'at","'Abasa","At-Takwir","Al-Infitar","Al-Mutaffifin","Al-Inshiqaq","Al-Buruj","At-Tariq","Al-A'la","Al-Ghashiyah","Al-Fajr","Al-Balad","Ash-Shams","Al-Lail","Ad-Dhuha","Ash-Sharh","At-Tin","Al-'Alaq","Al-Qadr","Al-Bayyinah","Az-Zalzalah","Al-'Adiyat","Al-Qari'ah","Al-Takathur","Al-Asr","Al-Humazah","Al-Fil","Quraysh","Al-Maun","Al-Kawthar","Al-Kafirun","An-Nasr","Al-Masad","Al-Ikhlas","Al-Falaq","An-Nas"];

// ENGLISCHE Alternate names (für .rt - unten, currently showing)
const ENGLISH_ALT = ["The Opener","The Cow","Family of Imran","The Women","The Table Spread","The Cattle","The Heights","The Spoils of War","The Repentance","Jonah","Hud","Joseph","The Thunder","Abraham","The Rocky Tract","The Bee","The Night Journey","The Cave","Mary","Ta-Ha","The Prophets","The Pilgrimage","The Believers","The Light","The Criterion","The Poets","The Ants","The Stories","The Spider","The Romans","Luqman","The Prostration","The Confederates","Saba","The Creator","Ya-Sin","Those Arranged in Ranks","Sad","The Troops","The Forgiver","Explained in Detail","Mutual Consultation","Ornament","Smoke","The Kneeling","The Sand Dunes","Muhammad","Victory","The Rooms","Qaf","The Wind Dispersing","The Mountain","The Star","The Moon","The Most Merciful","The Inevitable","Iron","She That Disputed","The Exodus","The Examined One","The Rank","Friday","The Hypocrites","Mutual Deception","Divorce","The Prohibition","The Kingdom","The Pen","The Inevitable Truth","The Stairs","Noah","The Jinn","The Enveloped One","The Covered One","The Resurrection","Man","Those Sent Forth","The News","Those Who Pull Out","He Frowned","The Coiling","The Splitting","The Defrauder","The Constellations","The Nightcomer","The Most High","The Overwhelming","The Dawn","The City","The Sun","The Night","The Morning Light","The Expansion","The Fig","The Clot","The Power","The Clear Evidence","The Earthquake","The Runners","Those Who Snatch Away","Those Who Run Swiftly","Those Who Bear Burdens","Time","The Slanderer","The Elephant","Quraysh","The Obligation","The Disbelievers","The Help","The Oneness","The Daybreak","Mankind"];

// ÜBERSETZUNGEN für .rt - unten
const SUREN = {
...
const NAV_TRANS = {
  es: { intro: 'Prefacio', back: 'Contraportada →', forward: '← Prefacio' },
  fr: { intro: 'Préface', back: 'Dos →', forward: '← Préface' },
  tl: { intro: 'Pambungad', back: 'Likod →', forward: '← Pambungad' },
  th: { intro: 'คำนำ', back: 'ปกหลัง →', forward: '← คำนำ' },
  kk: { intro: 'Кіріспе', back: 'Артқы қаб →', forward: '← Кіріспе' },
};
...
for (const [langName, langCode] of Object.entries(NEW_LANGS)) {
  ...
  // Fix Surah names: ENGLISH in .rs, TRANSLATION in .rt
  for (let i = 0; i < names.length; i++) {
    const pattern = `<span class="rs">${ENGLISH_ALT[i]}</span><span class="rt">${ENGLISH_ALT[i]}</span>`;
    const replacement = `<span class="rs">${ENGLISH_NAMES[i]}</span><span class="rt">${names[i]}</span>`;
    html = html.split(pattern).join(replacement);
  }
  
  // Fix navigation
  html = html.replace(`<a href="back-cover.html">Rückseite →</a>`, `<a href="back-cover.html">${NAV_TRANS[langCode].back}</a>`);
  html = html.replace(`<a href="intro.html">← Vorwort</a>`, `<a href="intro.html">← ${NAV_TRANS[langCode].intro}</a>`);
  
  fs.writeFileSync(indexPath, html, 'utf8');
  console.log(`✓ ${langName} index.html`);
  total++;
}
...
for (const [langName, langCode] of Object.entries(NEW_LANGS)) {
  ...
  // Fix navigation
  html = html.replace(`<a href="back-cover.html">Rückseite →</a>`, `<a href="back-cover.html">${NAV_TRANS[langCode].back}</a>`);
  html = html.replace(`<a href="index.html">← Inhaltsangabe</a>`, `<a href="index.html">← Inhaltsangabe</a>`);
  
  fs.writeFileSync(introPath, html, 'utf8');
  console.log(`✓ ${langName} intro.html`);
  total++;
}

// 3. Fix back-cover.html
for (const [langName, langCode] of Object.entries(NEW_LANGS)) {
  ...
  console.log(`✓ ${langName} back-cover.html`);
  total++;
}

console.log(`\n✅ ${total} files fixed (CORRECT struktur)`);
